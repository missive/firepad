...
firepad.EntityManager = (function () {
  ...
  function EntityManager() {
    ...
    this.register('table', {
      render: function(info) {
        utils.assert(info.outerHTML, "table entity should have 'outerHTML'!");
...
firepad.RichTextCodeMirror = (function () {
  ...
  RichTextCodeMirror.prototype.setAttribute = function(attribute, value) {
    ...
    // When updating href with an empty selection, update the whole link/word
    if (attribute == ATTR.HREF && this.emptySelection_()) {
      var pos = cm.indexFromPos(cm.getCursor('start'));
      var spans = this.annotationList_.getAnnotatedSpansForPos(pos);
      var span;

      if (span = spans[0]) {
        if (span.annotation.attributes[ATTR.HREF]) {
          cm.setSelection(cm.posFromIndex(span.pos), cm.posFromIndex(span.pos + span.length));
        } else {
          var wordPos = cm.findWordAt(cm.getCursor('start'));
